...
Программа реализована в виде исполняемого модуля **Struktura.exe**. Программа, предназначена для изучения структуры базы данных Айтида. Используется для просмотра наименования таблиц, имен полей и их описаний.

Запустить программу можно и рабочего каталога Айтиды, запустив исполняемый файл **Struktura.exe**.
...
**/nc17** - Наличие ключа указывает системе на необходимость использовать драйвер ODBC Driver for SQL Server 17 (поставляется вместе с MS SQL Server2019/2022) и имеет улучшенную производительность по сравнению с предыдущими версиями драйверов.

**/nc13** - Наличие ключа указывает системе на необходимость использовать драйвер ODBC Driver for SQL Server 13 (поставляется вместе с MS SQL Server2016/2017) и имеет улучшенную производительность по сравнению с предыдущими версиями драйверов.
...
**/xc** - Позволяет указать свой заголовок окна приложения

**/sv** - Позволяет игнорировать проверку версии базы при запуске приложения

**/sb** - Отключает выполнение резервной копии по расписанию при запуске системы

**/sp** - Позволяет установить картинку в главном окне приложения

**/sc** - Позволяет установить цвет фона главного окна

**/yhc** - Позволяет скрыть детали заголовка главного окна

**/ro** - Указывает на запуск только одного экземпляра приложения. При повторном запуске по ярлыку будет открываться не второй экземпляр, а активизироваться уже запущенный.

**/skipupdate** - Позволяет пропустить проверку наличия обновления
